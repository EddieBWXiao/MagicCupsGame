function [out]= gridsearch_RW2lr_1beta1stickiness(information,choice, start, alphabins,betabins, resp_made,fig_yes)
...
% Creates vector in linear space for the stickiness
s_label=-10:20/(betabins-1):10; %same length as betabins
...
% expand (repeat) the grid for beta value s & sticikiness values
...
% same as old val matrix on dim 4 and 5, ready for different beta & stick's

% matrix of beta and stickiness with same dimensions as value matrix
    % *** unique beta on dim 4 *** 
    % *** unique stickiness on dim 5 ***
betatot=repmat(permute(exp(b_label),[1 3 4 2 5]),[length(alphas) length(alphas) length(information) 1 length(s_label)]);
stickiness=repmat(permute(s_label,[1 3 4 5 2]),[length(alphas) length(alphas) length(information) length(b_label) 1]);

%% run response model 

%include choice stickiness
stickRaw = choice(1:end-1) == 1; %prelim: 1 if continue left, 0 if continue right
stickCode = double(stickRaw);
stickCode(~stickRaw) = -1; %swap to 1 and -1 code
stickCode = [0;stickCode]; %nothing to stick to initially 
    %also, the line above shifts the code to the right place (previous
    %trial influences the next trial...)
%make compatible with 5D matrix
stickLarge=repmat(permute(stickCode,[2 3 1 4 5]),[length(alphas) length(alphas)  1 length(b_label) length(s_label)]);
%choice'
%stickCode'

%then, get choice prob from value + stickiness
probleft=1./(1+exp(-(betatot.*(mmdl_win-mmdl_loss)+stickiness.*stickLarge)));
% result: 5D value matrix with all parameter combinations on all trials
% p(left_per_trial|alpha_w,alpha_l,beta,stickiness), 
    %sum to one with p(right_per_trial...)
clear betatot stickiness stickLarge
...
ch=repmat(permute(choice,[2 3 1 4 5]),[length(alphas) length(alphas)  1 length(b_label) length(s_label)]);
...
% creates four dimensional matrix (one dimension per free parameter)
...
% the stickiness
    out.marg_stickiness=squeeze(sum(sum(sum(out.posterior_prob,1),2),3));
    out.mean_stickiness=(s_label*out.marg_stickiness);
    out.var_stickiness=(((s_label-(out.mean_stickiness)).^2)*out.marg_stickiness);

out.beta_label=betalabel;
out.lr_label=alphalabel;
out.lr_points=alphas;
out.beta_points=b_label;
out.stickiness_points=s_label;
...
if fig_yes==1 
   ...
   title('Beta');
   subplot(2,2,4);
   plot(s_label,out.marg_stickiness);
  hold on
   xline(out.mean_stickiness)
   hold off
   legend('distribution','est stickiness')
   title('stickiness');
end
...
prob_ch_left=1./(1+exp(-(out.mean_beta.*(v(:,1)-v(:,2))+out.mean_stickiness*stickCode)));
...
out.BIC=(2.*out.neg_log_like)+4*(log(sum(resp_made))); % BIC given 4 free parameter;
...
xt = xt';
    if fig_yes==1
        ...
        xlabel('trials')
        ylabel('probability')
        title(sprintf('win lr = %.3f , loss lr = %.3f, beta = %.1f, stickiness = %.1f',...
            out.mean_alpha_win,out.mean_alpha_loss, out.mean_beta,out.mean_stickiness))
