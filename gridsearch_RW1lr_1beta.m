function [out]= gridsearch_RW1lr_1beta(information,choice, start, alphabins,betabins, resp_made,fig_yes)
...
for cycle1 = length(a_range):-1:1
    for cycle2 = length(b_range):-1:1
        ...
        joint(cycle1,cycle2) = get_IBLT_RW1lr_beta_lik(parameters,all_actions,all_outcomes);
    end
end
...
[mlogL,v,pchoice]=get_IBLT_RW1lr_beta_lik([logit(out.mean_alpha) log(out.mean_beta)],all_actions,all_outcomes);%logL + value expectation trajectory
...
if fig_yes==1
    ...
    plot(xt,choice,'*');
    hold off
    legend('value expectation for opt 1','p(choose option 1)','chosen option 1')
    xlabel('trials')
    ylabel('probability')
    title(sprintf('lr = %.3f , beta = %.1f',...
...
function [loglik,vv,pchoice] = get_IBLT_RW1lr_beta_lik(parameters, actions, outcomes_all)
