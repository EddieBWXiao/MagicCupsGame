function simu = RW1lr_multiblock_plsim(task,params,condition,graph)
...
% have three different learning rates for three different blocks
% same inverse temperature

input:
% task: task structure, from gen_misc
% params: vector, with the values
% condition: label each trial as block 1, 2, or 3
...
%% apply model (for belief update)
    ...
    alpha3 = params(3);
    beta = params(4);
    params = struct('alpha1',alpha1,'alpha2',alpha2,'alpha3',alpha3,'beta',beta);
    model_title = sprintf('alpha1 = %.3f, alpha2 = %.3f, alpha3 = %.3f, beta = %.1f',alpha1,alpha2,alpha3, beta);
    ...
    for t=1:nt
        ...
        if condition(t) == 1
            v(t+1,:) = v(t,:) + alpha1*PE(t,:);
        elseif condition(t) == 2
            v(t+1,:) = v(t,:) + alpha2*PE(t,:);
        elseif condition(t) == 3
            v(t+1,:) = v(t,:) + alpha3*PE(t,:);
        end
