function [negLL,vv,p]=lik_RW_Goris_RW3lr1beta_reset(parameters, actions, outcomes,condition,priors)

%{
%fit three different blocks, with same inverse temperature
% done in native space
% note: value from previous block does NOT persist to the next
...
for t=1:T 
    ...
    % allow reset of value expectations at beginning of each block
    if t ==1
        v(t,:) = v_trial;
    else
        if condition(t)==condition(t-1)
            v(t,:) = v_trial;
        else
            v(t,:) = v_initial;
        end
