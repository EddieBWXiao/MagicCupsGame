function [out]= gridsearch_RW2lr_1betaplus(information,choice, start, alphabins,betabins, resp_made,fig_yes)
...
% expand (repeat) the grid for beta value s & addm values
...
% same as old val matrix on dim 4 and 5, ready for different beta & addm's

% matrix of beta and addm with same dimensions as value matrix
    % *** unique beta on dim 4 *** 
    % *** unique addm on dim 5 ***
betatot=repmat(permute(exp(b_label),[1 3 4 2 5]),[length(alphas) length(alphas) length(information) 1 length(add_label)]);
addam=repmat(permute(add_label,[1 3 4 5 2]),[length(alphas) length(alphas) length(information) length(b_label) 1]);

%% run response model 
probleft=1./(1+exp(-(betatot.*(mmdl_win+addam-mmdl_loss))));
% result: 5D value matrix with all parameter combinations on all trials
% p(left_per_trial|alpha_w,alpha_l,beta,addam), 
...
ch=repmat(permute(choice,[2 3 1 4 5]),[length(alphas) length(alphas)  1 length(b_label) length(add_label)]);
...
% added constant (bias to one choice)
    out.marg_addm=squeeze(sum(sum(sum(out.posterior_prob,1),2),3));
...
prob_ch_left=1./(1+exp(-(out.mean_beta.*(v(:,1)+out.mean_addm-v(:,2)))));
...
xt = xt';
    if fig_yes==1
        ...
        xlabel('trials')
        ylabel('probability')
        title(sprintf('win lr = %.3f , loss lr = %.3f, beta = %.1f, added-bias = %.3f',...
            out.mean_alpha_win,out.mean_alpha_loss, out.mean_beta,out.mean_addm))
