function [actions, traj] = simu_RW2lr_1beta1lapse(params,opt1)

% simplified simulation code for IBLT task
% model: Rescorla-Wagner, two learning rates, with one beta, & lapse rate
% input:
...
for t=1:nt
    ...
    pchoice_opt1 = ((1+exp(-beta*(v_w(t)-v_l(t)))).^-1)*(1-lapse)+lapse/2;
...
%% output
params = struct('alpha_w',alpha_w,'alpha_l',alpha_l,'beta',beta,'lapse',lapse);
