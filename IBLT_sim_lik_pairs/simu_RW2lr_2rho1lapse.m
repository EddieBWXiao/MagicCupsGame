function [actions, traj] = simu_RW2lr_2rho1lapse(params,opt1)

% simplified simulation code for IBLT task
% model: Rescorla-Wagner, two learning rates, with different sensitivity to win & loss, has lapse rate
% input:
    % params: parameters; 5x1 matrix, a_win a_loss, rho_win, rho_loss, lapse
...
for t=1:nt
    ...
    pchoice_opt1 = ((1+exp(-(v_w(t)-v_l(t)))).^-1)*(1-lapse)+lapse/2;
    ...
    %% learning
    WPE(t) = rho_w*wins(t) - v_w(t);%calculate PE
    LPE(t) = rho_l*losses(t) - v_l(t);
        %important: outcome multiplied by sensitivity, rho
...
%% output
params = struct('alpha_w',alpha_w,'alpha_l',alpha_l,'rho_w',rho_w,'rho_l',rho_l,'lapse',lapse);
