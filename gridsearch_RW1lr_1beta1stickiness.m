function [out]= gridsearch_RW1lr_1beta1stickiness(information,choice, start, alphabins,betabins, resp_made,fig_yes)
...
mmdl_v=repmat(val_l,[1,1,length(b_label),length(s_label)]);%nbins x ntrials x nbins x nbins
clear val_l

% matrix of beta with same dimensions as value matrix
    % *** unique stickiness on dim 4 ***
    % *** unique beta on dim 3 *** 
betatot=repmat(permute(exp(b_label),[1 3 2 4]),... %move dim 2 to dim 3
    [length(alphas) length(information) 1, length(s_label)]);
stickiness=repmat(permute(s_label,[1 3 4 2]),...%move dim 2 to dim 4
...
stickLarge=repmat(permute(stickCode,[2 1 3 4]),[length(alphas) 1 length(b_label) length(s_label)]);

probleft=1./(1+exp(-(betatot.*(mmdl_v)+stickiness.*stickLarge)));
...
ch=repmat(permute(choice,[2 1 3 4]),[length(alphas) 1 length(b_label) length(s_label)]);
...
out.posterior_prob=out.posterior_prob./(sum(sum(sum(out.posterior_prob))));% normalise (divide by sum over entire 4D grid space)
    ...
    %dim1 LR, dim2 beta, dim3 stickiness
...
%stickiness
out.marg_stickiness=squeeze(sum(sum(out.posterior_prob,1),2));
...
prob_ch_left=1./(1+exp(-(out.mean_beta.*v(:,1)+out.mean_stickiness*stickCode)));
...
xt = xt';
    if fig_yes==1
        ...
        xlabel('trials')
        ylabel('probability')
        title(sprintf('lr = %.3f , beta = %.1f, stickiness = %.3f',...
            out.mean_alpha, out.mean_beta,out.mean_stickiness))
    end
