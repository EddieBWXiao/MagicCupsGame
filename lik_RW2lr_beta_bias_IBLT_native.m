function [negLL,vv,pchoice] = lik_RW2lr_beta_bias_IBLT_native(parameters, actions, opt1)
...
% opt1: outcomes from only opt1 (see simu_RW2lr_bias)
...
beta  = parameters(3);
%bias: preference for the reference option
bias = parameters(4);%linear, usually from -2 to 2
...
for t=1:T    
    ...
    %% response model
    p1 = 1./(1+exp(-beta*(v_w(t)-v_l(t)+bias))); % probability of action 1
